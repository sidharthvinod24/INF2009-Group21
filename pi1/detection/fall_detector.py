...
class FallTracker:
    ...
    def __init__(self, pid):
        ...
        self._not_horizontal_count   = 0   # hysteresis: frames without horizontal pose
    ...
    def update(self, is_horizontal, cur_angle, mode, kps_snap):
        ...
        if not is_horizontal:
            # Once a fall/lying state is confirmed, tolerate brief bad frames
            # from YOLO (lying poses are harder to estimate).  Only reset after
            # 8 consecutive non-horizontal frames (~0.5 s at 15 fps).
            if self.confirmed_state in ('fall', 'lying'):
                self._not_horizontal_count += 1
                if self._not_horizontal_count < 8:
                    return self.state   # stay in confirmed state
            self._not_horizontal_count = 0
            self._reset_to_safe()
            return self.state
        self._not_horizontal_count = 0
